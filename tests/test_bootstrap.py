...
LEGACY_MARKED = (
    "---\ndescription: old\n---\n"
    "<!-- gingugu-init:managed-file - regenerated by `gingugu init`; "
    "local edits are lost on --force -->\n\n## Sink the Ship\n"
)
LEGACY_HAND_WRITTEN = "---\ndescription: mine\n---\n\n## My own sink command\n"
...
def test_retires_our_own_legacy_command_and_keeps_a_backup(tmp_path):
    """The duplicate is the bug: two definitions answering to one name."""
    legacy = _legacy_command(tmp_path)
    legacy.write_text(LEGACY_MARKED)

    assert main(["--path", str(tmp_path)]) == 0

    assert not legacy.exists()
    assert (tmp_path / ".claude" / "skills" / "sink-the-ship" / "SKILL.md").exists()
    # Ours or not, the bytes survive. Ownership is not a licence to delete.
    assert _read(legacy.parent / "sink-the-ship.md.bak") == LEGACY_MARKED


def test_retirement_needs_no_force(tmp_path):
    """A plain run fixes the duplicate, because the .bak means nothing is lost."""
    legacy = _legacy_command(tmp_path)
    legacy.write_text(LEGACY_MARKED)
...
def test_never_deletes_a_command_we_did_not_write(tmp_path):
    """A nuisance duplicate is recoverable. Deleting someone's work is not."""
    legacy = _legacy_command(tmp_path)
    legacy.write_text(LEGACY_HAND_WRITTEN)

    assert main(["--path", str(tmp_path)]) == 0

    assert legacy.exists()
    assert _read(legacy) == LEGACY_HAND_WRITTEN
...
def test_dry_run_retires_nothing(tmp_path):
    ...
    assert legacy.exists()
    assert _read(legacy) == LEGACY_MARKED
    assert not (legacy.parent / "sink-the-ship.md.bak").exists()
