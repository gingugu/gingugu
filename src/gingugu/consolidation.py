...
Because a consolidation is ``1 + 2N`` writes, ``consolidate()`` runs inside
``transactions.atomic()``: it applies whole or not at all. Without it a failure
partway through the retirement loop leaves some originals retired and some not,
and under ``keep_originals=False`` that is unrecoverable.

Finding clusters worth consolidating is the read-only half and lives in
``duplicate_scan.py``.
"""
    ...
    *,
    ...
    if strategy == "deduplicate":
        ...
        with atomic(store, relations):
            # Fold the union in BEFORE retiring anything. With
            # ``keep_originals=False`` the retirement is a hard delete and the
            # losers' tags exist nowhere else, so the reverse order loses them
            # outright if the loop fails partway.
