    ...
    try:
        ...
        if args.verbose:
            if trigger == "manual":
                sid = session_id[:8]
                message = (
                    "Preparing for manual compaction"
                    f" (session: {sid}...)"
                )
                if custom_instructions:
                    instr = custom_instructions[:100]
                    message += f"\nCustom instructions: {instr}..."
            else:  # auto
                sid = session_id[:8]
                message = (
                    "Auto-compaction triggered due to"
                    f" full context window (session: {sid}...)"
                )
